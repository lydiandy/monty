...
impl MontyRepl {
    ...
    /// Like [`Self::feed_start`], compiling `host_modules` with the snippet
    pub fn feed_start_with_host_modules(
        ...
        match self.check_source(code) {
            Ok(checked_code) => self.feed_start_checked_with_host_modules(checked_code, inputs, print, host_modules),
            Err(error) => Err(Box::new(ReplStartError { repl: self, error })),
        }
    }

    /// [`feed_start`](Self::feed_start) for a snippet [`check_source`](Self::check_source) already vetted
    ///
    /// # Errors
    ...
    pub fn feed_start_checked(
        ...
        self.feed_start_checked_with_host_modules(code, inputs, print, Vec::new())
    }

    /// [`feed_start_checked`](Self::feed_start_checked) plus app-dir host modules
    pub fn feed_start_checked_with_host_modules(
        self,
        code: CheckedSource<'_>,
        inputs: impl Into<NamedValues>,
        print: PrintWriter<'_>,
        ...
        for module in &host_modules {
            this.sources
                .insert(module.filename.clone(), Arc::from(module.source.as_str()));
        }
