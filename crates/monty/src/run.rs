...
pub(crate) struct Executor {
    ...
    heap_capacity: usize,
    /// Compiled app-dir modules. Not part of a dump
    #[serde(skip)]
...
impl Executor {
    ...
    /// Compiles the entry module and host app-dir modules in one intern pool
    pub(crate) fn new_with_host_modules(
        ...
        for module in &extra {
            source_nesting_exception(&module.source, &module.filename, options.source_scan_threshold)?;
        }
        let mut interns = Interns::new(&code);
        let mut globals = NameMap::new();
        let (module_code, _, host_specs) = compile_module_source(
            &code,
            script_name,
            &mut globals,
...
fn compile_module_source(
    ...
    options: CompileOptions,
    extra: Vec<HostModuleSource>,
) -> Result<(Code, Vec<NamespaceId>, HashMap<String, HostModuleSpec>), MontyException> {
    ...
    let mut host_specs = HashMap::new();
    for module in extra {
        interns.intern(&module.name);
        let nodes = parse_with_interner(&module.source, &module.filename, &mut interns)
            .map_err(|e| e.into_python_exc(&module.filename, &module.source))?;
        let mut module_globals = NameMap::new();
        let nodes = prepare_with_existing_names(nodes, &interns, &mut module_globals)
            .map_err(|e| e.into_python_exc(&module.filename, &module.source))?;
        let start = u32::try_from(interns.functions_len()).unwrap_or(u32::MAX);
        let module_code = Compiler::compile_module(&nodes, &mut interns, &module_globals, options)
            .map_err(|e| e.into_python_exc(&module.filename, &module.source))?;
        let end = u32::try_from(interns.functions_len()).unwrap_or(u32::MAX);
        host_specs.insert(
            module.name,
            HostModuleSpec {
