...
use crate::{
    ...
    types::{
        Dict, LongInt, Module, PyTrait, SessionRandom,
...
impl<'code> CallFrame<'code> {
    ...
    fn new_nested_module(code: &'code Code, exception_stack_base: usize, stack_base: usize) -> Self {
        ...
        frame.stack_base = stack_index(stack_base);
...
impl<'h> VM<'h> {
    ...
    /// Like [`Self::new`], but the module frame is parked so `evaluate_function` does not resume it
    pub(crate) fn new_parked(
        globals: Vec<Value>,
        tables: &'h mut SessionTables,
        program: &'h Program,
        heap: &'h mut HeapReader<'h>,
        ...
        let code: &'h Code = &program.module_code;
        let mut vm = Self::new(globals, tables, program, heap, print_writer);
        vm.current_frame = CallFrame::new_parked(code);
        vm
    }

    /// Installs the embedder's compiled app-dir modules for host imports
    ...
    fn load_module(&mut self, module_id: u16) -> RunResult<()> {
        ...
        if let Some(module) = self.interns.static_string(name_id).and_then(StandardLib::from_static) {
            ...
            self.load_host_module(name_id)
        }
    }

    /// Unknown stdlib name: cached host module, native vtable, app-dir `.py`, then `ModuleNotFoundError`
    fn load_host_module(&mut self, name_id: StringId) -> RunResult<()> {
        ...
        let module_obj = Module::from_name_id(name_id);
