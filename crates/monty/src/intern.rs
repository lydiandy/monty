...
impl Interns {
    ...
    pub fn new(code: &str) -> Self {
        ...
        // gpui-monty module names stay out of `StaticStrings`, after the core pool
        for name in ["ui", "gpui", "gpui_base", "db", "__gpui_view__"] {
            let id = next_string_id(interns.strings.len());
            interns.strings.push(InternedString::owned(name.to_owned()));
            interns.string_id_by_name.borrow_mut().insert(name.to_owned(), id);
        }
        interns
    }
