...
impl Interns {
    pub fn new(mut interner: InternerBuilder, functions: Vec<Function>) -> Self {
        // gpui-monty host module names. Kept out of `StaticStrings` so widget
        // identifiers do not append to the dump-stable enum tail. `intern` is a
        // no-op when parse already saw the name; otherwise they land at the end
        // of the dynamic pool and do not shift existing ids.
        for name in ["ui", "gpui", "gpui_base", "db", "__gpui_view__"] {
